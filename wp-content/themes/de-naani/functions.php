...
function de_naani_customize_css(){
	...
	if($de_naani_options['title_position'] == 'center'){
		echo ".site-header h1, .site-header h2 { text-align:center; }";
	}
	if($de_naani_options['title_position'] == 'right'){
...
EOM;
			...
			foreach($this->choices as $key=>$val){
				...
				echo "<label class=\"ptn_".esc_html($key)."\" style=\"background-image:url('".get_bloginfo('wpurl')."/wp-content/themes/de_naani/images/ptn_".esc_html($key).".png')\">";
